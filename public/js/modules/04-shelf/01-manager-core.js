  ...
  function currentItems() {
    if (hasAnyPlatformLogin() && (userPlaylists.length || myPodcastCollections.length)) {
      ...
      var items = source.map(function (pl) {
        var provider = pl.provider === 'qq' ? 'qq' : (pl.provider === 'kugou' ? 'kugou' : (pl.provider === 'qishui' ? 'qishui' : (pl.provider === 'spotify' ? 'spotify' : (pl.provider === 'apple' ? 'apple' : 'netease'))));
        var sourceLabel = provider === 'qq' ? 'QQ' : (provider === 'kugou' ? 'KG' : (provider === 'qishui' ? 'QS' : (provider === 'spotify' ? 'SP' : (provider === 'apple' ? 'AM' : 'NE'))));
